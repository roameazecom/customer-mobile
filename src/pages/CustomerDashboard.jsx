...
export default function CustomerDashboard() {
  ...
  const fetchCustomerHistory = async (phoneNum) => {
    ...
    if (!phone) return;
    try {
      const baseUrl = usePosStore.getState().getServerUrl();
      const response = await axios.get(`${baseUrl}/orders/customer/history?phone=${phone}`);
                ...
                <>
                  ...
                  <div className="mt-4 flex gap-2">
                    <button
                      onClick={() => {
                        const baseUrl = usePosStore.getState().getServerUrl();
                        window.open(`${baseUrl}/reports/invoice/${activeOrder.id}`, '_blank');
                      }}
                    ...
                    <div key={order.id} className="p-4 rounded-3xl border border-slate-100 bg-slate-50/50 space-y-3">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-slate-100">
                        ...
                        <button
                          onClick={() => {
                            const baseUrl = usePosStore.getState().getServerUrl();
                            window.open(`${baseUrl}/reports/invoice/${order.id}`, '_blank');
                          }}
